...
void main() {
  test('a body survives being read back through its own table', () async {
    ...
    await db.into(db.actions).insert(
    ...
    await db.into(db.actionBodies).insert(
    ...
    final body = await (db.select(db.actionBodies)
          ..where((b) => b.actionId.equals('a1')))
        .getSingleOrNull();
  ...
  test('an action without a body is a readable row, not an error', () async {
    ...
    final row = await (db.select(db.actions)..where((a) => a.id.equals('a2')))
        .getSingleOrNull();
    expect(row, isNotNull, reason: 'a locked action is still a row');

    final body = await (db.select(db.actionBodies)
          ..where((b) => b.actionId.equals('a2')))
        .getSingleOrNull();
    expect(body, isNull);
  });
}
